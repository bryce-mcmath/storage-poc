...
import { secureStorageService, storageService } from './new/storage'
...
export const migrateFromExistingToNew = async (): Promise<boolean> => {
  try {
    ...
    if (secret) {
      ...
      const settings = await existingStorageService.getItem<SettingsState>(
        'app_settings',
      )
      const biometricsEnabled = settings?.biometricsEnabled || false

      // Store wallet secret in new implementation
      const walletSecretMigrated = await secureStorageService.storeWalletSecret(
        secret,
        biometricsEnabled,
      )
    ...
    console.log(
      'Migration completed with status:',
      migrationSuccess ? 'success' : 'partial failure',
    )
